...
function temp_main(inputDataPath,boxBorder,tminStack,tmaxStack,tasStack,region)
	...
	for row = 1:allRows
		for col = 1:(boxBorder(1,5)-boxBorder(1,4)+1)
			...
			[tasPart.lgm.mean(row,col), tasPart.midH.mean(row,col), tasPart.lgm.std(row,col), tasPart.midH.std(row,col)] = ...
				 quantileMatch(dummyTseries,tas_histBlock(row,col,:), tas_lgmBlock(row,col,:),tas_midHBlock(row,col,:),[]);

			[tminPart.lgm.mean(row,col), tminPart.midH.mean(row,col), tminPart.lgm.std(row,col), tminPart.midH.std(row,col)] = ... 
				quantileMatch(tmin_obsStack(row,col,:), tasmin_histBlock(row,col,:), tasmin_lgmBlock(row,col,:),tasmin_midHBlock(row,col,:),[]);

			[tmaxPart.lgm.mean(row,col), tmaxPart.midH.mean(row,col), tmaxPart.lgm.std(row,col), tmaxPart.midH.std(row,col)] = ...
